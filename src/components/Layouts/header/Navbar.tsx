...
const Navbar = () => {
  ...
  return (
    ...
    <div className="flex flex-wrap items-center justify-between gap-5 w-full">
      <a href="javascript:void(0)" className="max-sm:hidden">Finance App</a>
      <a href="javascript:void(0)" className="hidden max-sm:block">Finance App</a>
      ...
      <div className="flex max-lg:ml-auto space-x-4">
        <Link href={status === 'authenticated' ? "/dashboard" : "/auth/sign-in"}>
            <button
              className="px-4 py-2 text-sm rounded-full font-medium cursor-pointer tracking-wide text-slate-900 border border-gray-400 bg-transparent hover:bg-gray-50 transition-all">{status === 'authenticated' ? 'Dashboard' : 'Login'}</button>
        </Link>
        <Link href={status === 'authenticated' ? "/auth/sign-out" : "/auth/sign-up"}>
          <button
            ...
            {status === 'authenticated' ? 'Logout' : 'Sign up'}</button>
        </Link>
